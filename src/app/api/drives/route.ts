...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate reports exist and are eligible
    if (reportIds.length > 0) {
      ...
      // 🚨 Check that each report has required status
    ...
    const participant = Number.isFinite(Number(body.participant))
      ? Number(body.participant)
      : 0;

    // ✅ Create the drive and link reports via nested create
    ...
    if (err instanceof Prisma.PrismaClientKnownRequestError) {
      return NextResponse.json(
        { error: "Database error creating drive" },
        { status: 500 }
      );
    }
