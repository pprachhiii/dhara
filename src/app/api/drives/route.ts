...
interface TaskInput {
  comfort: Prisma.TaskCreateManyInput["comfort"]; 
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = {
      ...
      proposedDate: drive.startDate.toISOString(),
      reports: drive.reports.map(r => ({ reportId: r.reportId })),
      taskBreakdown: drive.tasks.map(task => ({
