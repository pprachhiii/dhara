  ...
  try {
    ...
    const report = await prisma.report.findUnique({
      ...
      include: {
        reporter: true,
        reportAuthorities: {
          include: { authority: true, volunteer: true },
        },
...
export async function PATCH(request: NextRequest, context: Context) {
  ...
  try {
    ...
    if (typeof data.longitude === 'number' || data.longitude === null) {
      updateData.longitude = data.longitude;
    }

    if (typeof data.city === 'string' || data.city === null) {
      updateData.city = data.city;
    }

    if (typeof data.region === 'string' || data.region === null) {
    ...
    if (typeof data.pinCode === 'string' || data.pinCode === null) {
...
export async function DELETE(request: NextRequest, context: Context) {
  ...
  try {
    ...
    await prisma.discussion.deleteMany({ where: { reportId: id } });
    await prisma.statusLog.deleteMany({ where: { reportId: id } });
    await prisma.vote.deleteMany({ where: { reportId: id } });
